...
// const getRepsValues = async (req, res) => {
//   const user_id = req.user._id;

//   try {
//     // Find all workouts for the user and project only the reps field
//     const workouts = await Workout.find({ user_id }).select('load');
//     console.log('Workouts:', workouts); // Log the retrieved workouts

//     // Extract only the reps values from each workout document
//     const repsValues = workouts.map(workout => workout.load);
//     console.log('Reps Values:', repsValues); // Log the extracted reps values

//     res.status(200).json(repsValues);
//   } catch (error) {
...
const getRepsValues = async (req, res) => {
  ...
  try {
    ...
    fs.writeFileSync('repsValues.json', JSON.stringify(repsValues));
...
const createWorkout = async (req, res) => {
  ...
  const user_id = "660267f6fdac4cf92697f236";
